function nixup
    ...
    function _nixup_ensure_git_repo
        ...
        if not git -C "$target_dir" rev-parse --is-inside-work-tree >/dev/null 2>&1
            ...
            git -C "$target_dir" init \
                ...
                and git -C "$target_dir" commit -m "feat(nixos): initial commit"
            if test $status -ne 0
    ...
    function _nixup_ensure_sudo
        sudo -v
        if test $status -ne 0
            echo (set_color red)"Sudo authentication failed."(set_color normal)
            return 1
        end
    end
    ...
    function _nixup_git_sync
        ...
        echo ""
        echo (set_color yellow)"Committing changes..."(set_color normal)
        ...
        if $should_push
            ...
            echo (set_color green)"Git commit and push successful!"(set_color normal)
        else
            echo (set_color green)"Git commit successful! (Use --push next time or 'nixup sync' to push)"(set_color normal)
        end
    end
    ...
    function _nixup_help
        ...
        echo "Usage: nixup [command] [--commit | --push] [--update] [\"message\"]"
        echo
        echo "Manages NixOS configuration with Git integration."
        echo
        echo "Commands:"
        _print_help_entry apply "Stage & apply. Options: --commit, --push (implies commit), --update."
        _print_help_entry boot "Stage & build boot. Options: --commit, --push, --update."
        ...
        _print_help_entry gc "Run garbage collection (keeps last 5 generations)."
        ...
        _print_help_entry test "Stage & test. Options: --commit, --push, --update."
    ...
    if contains -- --commit $argv; or $should_push
    ...
    if contains $command apply boot gc secrets sync test
        _nixup_ensure_sudo; or return 1
    end
    ...
    switch $command
        ...
        case apply boot test
            ...
            if $should_commit
                ...
                _nixup_git_sync "$commit_message" "$config_dir" "$secrets_dir" $should_push
                return $status
            end
        case cd
            cd "$config_dir"
        case config
            ...
            return 0
        case gc
            echo (set_color yellow)"Running garbage collection (keeping last 5 generations)..."(set_color normal)
            nh clean all --keep 5
            return $status
        case lazygit
        ...
        case sync
            ...
            _nixup_git_sync "$commit_message" "$config_dir" "$secrets_dir" true
            return $status
        case '*'
